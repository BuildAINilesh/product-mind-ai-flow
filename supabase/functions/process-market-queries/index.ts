...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Using Firecrawl API Key: ${
        firecrawlApiKey ? "Available (masked)" : "Missing"
      }`
    );

    if (!firecrawlApiKey) {
      throw new Error(
        "Firecrawl API Key is missing. Please set it in the Supabase Edge Function Secrets."
      );
    }

    // Get all pending queries for the requirement
    const queriesResponse = await fetch(
      `${supabaseUrl}/rest/v1/firecrawl_queries?requirement_id=eq.${requirementId}&status=eq.pending`,
      {
        method: "GET",
        headers: {
    ...
    for (const query of queries) {
      ...
      try {
        ...
        const { response: searchResponse, error: fetchError } =
          await fetchWithRetry(apiEndpoint, searchRequest, 3, 2000);

        if (fetchError) {
          console.error(
            `Fetch error for query '${query.query}': ${fetchError}`
          );
          ...
          await fetch(
            `${supabaseUrl}/rest/v1/firecrawl_queries?id=eq.${query.id}`,
            {
              method: "PATCH",
              headers: {
                Authorization: `Bearer ${supabaseServiceKey}`,
        ...
        if (!searchResponse.ok) {
          ...
          console.error(
            `Firecrawl search error for query '${query.query}': ${errorText}`
          );
          console.error(
            `Status: ${searchResponse.status}, Headers: ${JSON.stringify([
        ...
        if (
          ...
          console.log(
            `Received ${results.length} results for query: ${query.query}`
          );
          ...
          if (results.length > 0) {
            ...
            for (const result of results) {
              console.log(
                `Saving result: ${result.title || "No Title"} | ${
                  result.url || "No URL"
                }`
              );
              ...
              const sourceResponse = await fetch(
                `${supabaseUrl}/rest/v1/market_research_sources`,
                {
                  method: "POST",
                  headers: {
                    Authorization: `Bearer ${supabaseServiceKey}`,
              ...
              if (!sourceResponse.ok) {
                ...
                console.error(
                  `Error storing search result: ${JSON.stringify(errorData)}`
                );
                continue;
              }
            ...
            // Mark the query as 'no_results' without creating placeholder records
            await fetch(
              `${supabaseUrl}/rest/v1/firecrawl_queries?id=eq.${query.id}`,
              {
                method: "PATCH",
          ...
          console.error(
            `Invalid response format from Firecrawl API: ${JSON.stringify(
              searchResults
            )}`
          );
        }
        ...
        if (queries.length > 1) {
          ...
          console.log(
            `Adding delay of ${delayMs}ms between requests to avoid rate limits`
          );
          await sleep(delayMs);
        }
      } catch (queryError) {
        console.error(
          `Error processing query '${query.query}': ${queryError.message}`
        );
    ...
    return new Response(
      JSON.stringify({
        ...
        message: `Processed ${processedQueries} queries and saved ${savedSources} search results${
          ...
          rateLimitHits > 0 ? ` (hit rate limits ${rateLimitHits} times)` : ""
        }`,
        processedQueries,
        savedSources,
        errorCount,
    ...
    return new Response(
      JSON.stringify({
        success: false,
        message:
          error.message || "An error occurred while processing market queries",
      }),
      {
        status: 500,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
