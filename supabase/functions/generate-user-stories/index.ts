...
serve(async (req) => {
  ...
  try {
    ...
    const { data: analysis, error: analysisError } = await supabase
      ...
      .select(
        "project_overview, problem_statement, proposed_solution, key_features"
      )
      .eq("requirement_id", requirementId)
      .maybeSingle();
    if (analysisError) {
    ...
    const projectOverview =
      ...
      requirement.project_idea ||
      requirement.project_name;
    const problemStatement =
      analysis?.problem_statement || "No specific problem statement available";
    const proposedSolution =
      analysis?.proposed_solution ||
      "No specific solution description available";
    const keyFeatures =
    ...
    const response = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a product manager tasked with transforming product requirements into clear, actionable user stories with acceptance criteria.

Each user story should follow the format:
"As a [user role], I want to [action] so that [benefit]"

Each story should include specific acceptance criteria that define when the story is considered complete.
...
Output (Strict JSON Array):
{
  "user_stories": [
    ...
    if (
      !userStoriesData.user_stories ||
      !Array.isArray(userStoriesData.user_stories)
    ) {
      ...
      return new Response(
        JSON.stringify({
          error: "Invalid user stories format in response",
        }),
        {
          status: 500,
          headers: {
    ...
    const userStoriesForDB = userStoriesData.user_stories.map((story) => ({
