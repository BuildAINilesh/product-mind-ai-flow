...
export const useAnalysisProgress = (
  ...
  const checkOngoingAnalysisProcess = () => {
    ...
    try {
      ...
      const currentStepSaved = localStorage.getItem(
        ANALYSIS_CURRENT_STEP_KEY + requirementId
      );
      const stepsSaved = localStorage.getItem(
  ...
  // Reset analysis progress - clears localStorage and resets UI state
  const resetAnalysisProgress = () => {
    if (!requirementId) return;

    console.log("Resetting analysis progress for requirement:", requirementId);
